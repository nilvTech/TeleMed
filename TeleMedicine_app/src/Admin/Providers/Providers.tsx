...
const INITIAL_PROVIDERS: Provider[] = [
  {
    ...
    availability: "Available",
    email: "john.smith@telehealth.com",
    licenseExpiry: "2025-08-15",
    deaNumber: "BS1234563",
    boardCertification: "ABIM",
  ...
  },
  {
    ...
    deaNumber: "BJ9876541",
    boardCertification: "ABD",
    malpracticeInsurance: "Active",
    credentialStatus: "Expiring Soon",
    todaySchedule: "10:00 AM – 3:00 PM",
    nextSlot: "11:00 AM",
    timezone: "CST",
  },
  {
    ...
    availability: "Available",
    email: "michael.chen@telehealth.com",
    submittedDate: "2024-01-10",
    documentStatus: "Complete",
    verificationStatus: "Pending",
    licenseExpiry: "2026-03-20",
    deaNumber: "BC5432107",
    boardCertification: "ABPN",
    malpracticeInsurance: "Active",
    credentialStatus: "Valid",
    todaySchedule: "–",
    nextSlot: "–",
  ...
  },
  {
    ...
    email: "emily.rodriguez@telehealth.com",
    licenseExpiry: "2025-11-30",
    deaNumber: "BR7654329",
    boardCertification: "ABFM",
    malpracticeInsurance: "Active",
    credentialStatus: "Valid",
    todaySchedule: "–",
    nextSlot: "–",
  ...
  },
  {
    ...
    availability: "Offline",
    email: "james.williams@telehealth.com",
    suspendedDate: "2024-01-05",
    suspendedReason: "License compliance issue",
    suspendedBy: "Admin Sarah K.",
    licenseExpiry: "2023-12-01",
    deaNumber: "BW2109876",
    boardCertification: "ABOS",
    malpracticeInsurance: "Expired",
  ...
  },
  {
    ...
    availability: "Available",
    email: "maria.lee@telehealth.com",
    licenseExpiry: "2026-07-10",
    deaNumber: "BP3214567",
    boardCertification: "ABPN",
    malpracticeInsurance: "Active",
    credentialStatus: "Valid",
    todaySchedule: "8:00 AM – 4:00 PM",
    nextSlot: "1:00 PM",
    timezone: "CST",
  },
  {
    ...
    availability: "Available",
    email: "robert.kim@telehealth.com",
  ...
  },
  {
    ...
    availability: "Offline",
    email: "linda.torres@telehealth.com",
    licenseExpiry: "2025-09-22",
    deaNumber: "BT9876541",
    boardCertification: "ABP",
    malpracticeInsurance: "Active",
    credentialStatus: "Valid",
    timezone: "MST",
  },
...
const MOCK_ACTIVITY = [
  {
    ...
    time: "Jan 2, 2024 — 9:00 AM",
    icon: "✦",
  },
  {
    id: "a2",
    event: "Profile completed",
    time: "Jan 2, 2024 — 9:15 AM",
    icon: "✎",
  },
  {
    id: "a3",
    event: "Credentials submitted",
    time: "Jan 3, 2024 — 11:30 AM",
    icon: "⊕",
  },
  {
    id: "a4",
    event: "Password changed",
    time: "Jan 5, 2024 — 2:45 PM",
    icon: "⬡",
  },
  {
    id: "a5",
    event: "First appointment completed",
    time: "Jan 8, 2024 — 10:00 AM",
    icon: "✓",
  },
...
const Toast: React.FC<{ message: string; onDone: () => void }> = ({
  message,
  onDone,
}) => {
  useEffect(() => {
...
const Avatar: React.FC<{ initials: string; status?: AvailabilityStatus }> = ({
  initials,
  status,
}) => (
  <div className={styles.avatarWrap}>
    <div
      ...
      style={{ background: AVATAR_COLORS[initials] || "#4b5563" }}
    >
      {initials}
    </div>
    {status && (
      <span
        className={`${styles.avatarDot} ${status === "Available" ? styles.dotGreen : status === "On Leave" ? styles.dotYellow : styles.dotGray}`}
      />
    )}
  </div>
);
...
const Badge: React.FC<{ label: string; type: string }> = ({ label, type }) => (
  <span
    className={`${styles.badge} ${styles[`badge_${type.toLowerCase().replace(/\s+/g, "_")}`] || styles.badge_inactive}`}
  >
    {label}
  </span>
);
...
const EmptyState: React.FC<{ message?: string }> = ({
  ...
  <div className={styles.emptyState} role="status">
    ...
    >
      ...
      <path
        d="M20 36c0-4.418 3.582-8 8-8s8 3.582 8 8"
        stroke="#94a3b8"
        strokeWidth="2"
        strokeLinecap="round"
      />
...
const ConfirmDialog: React.FC<{
  ...
  <ModalShell title={title} onClose={onClose} size="sm">
    ...
    <div className={styles.modalActions}>
      ...
      <button
        className={confirmClass || styles.btnPrimary}
        onClick={() => {
          onConfirm();
          onClose();
        }}
...
const ProviderForm: React.FC<{
  ...
  const validate = () => {
    ...
    if (!form.npi.trim() || !/^\d{10}$/.test(form.npi))
  ...
  const renderField = (
    ...
    <div className={styles.formGroup} key={key}>
      <label className={styles.formLabel} htmlFor={`pf-${key}`}>
        {label}
      </label>
      <input
        id={`pf-${key}`}
      ...
      />
      {errors[key] && (
        <span id={`pf-err-${key}`} className={styles.formError} role="alert">
          {errors[key]}
        </span>
      )}
    </div>
  ...
  return (
    <ModalShell
      title={mode === "add" ? "Add New Provider" : "Edit Provider"}
      onClose={onClose}
      size="lg"
    >
    ...
    <ModalShell title="Send Notification" onClose={onClose}>
      <div className={styles.modalBody}>
        <p className={styles.modalSubtitle}>
          To: <strong>{provider.name}</strong> · {provider.email}
        </p>
        <div className={styles.formGroup}>
          <label className={styles.formLabel} htmlFor="notif-subject">
            Subject *
          </label>
          <input
            id="notif-subject"
        ...
        <div className={styles.formGroup}>
          <label className={styles.formLabel} htmlFor="notif-msg">
            Message *
          </label>
          <textarea
            id="notif-msg"
...
const ActivityModal: React.FC<{ provider: Provider; onClose: () => void }> = ({
  provider,
  onClose,
}) => (
  <ModalShell title="Provider Activity" onClose={onClose}>
    <div className={styles.modalBody}>
      <p className={styles.modalSubtitle}>
        {provider.name} — Activity Timeline
      </p>
...
const ViewDocsModal: React.FC<{ provider: Provider; onClose: () => void }> = ({
  provider,
  onClose,
}) => (
  <ModalShell title="Provider Documents" onClose={onClose}>
    <div className={styles.modalBody}>
      <p className={styles.modalSubtitle}>
        {provider.name} — Submitted Documents
      </p>
        ...
        {MOCK_DOCUMENTS.map((d) => (
          <div key={d.id} className={styles.docItem}>
            ...
            <div className={styles.docRight}>
              <span
                className={`${styles.docStatusBadge} ${d.status === "Verified" ? styles.docVerified : styles.docPending}`}
              >
                {d.status}
              </span>
              <button
...
const RequestInfoModal: React.FC<{
  ...
  return (
    <ModalShell title="Request Additional Information" onClose={onClose}>
      <div className={styles.modalBody}>
        <p className={styles.modalSubtitle}>
          Requesting info from <strong>{provider.name}</strong>
        </p>
        <div className={styles.formGroup}>
          <label className={styles.formLabel} htmlFor="req-msg">
            Message *
          </label>
          <textarea
            id="req-msg"
...
const RejectModal: React.FC<{
  ...
  return (
    <ModalShell title="Reject Provider" onClose={onClose} size="sm">
      <div className={styles.modalBody}>
        <p className={styles.modalSubtitle}>
          Rejecting <strong>{provider.name}</strong>. This will remove them from
          the approval queue.
        </p>
        <div className={styles.formGroup}>
  ...
  return (
    ...
    >
      {items.map(({ label, icon, action, danger }) => (
        <button
          key={label}
          className={`${styles.moreMenuItem} ${danger ? styles.moreMenuItemDanger : ""}`}
          role="menuitem"
          onClick={(e) => {
            e.stopPropagation();
            action();
  ...
  return (
    <div className={styles.pagination}>
      ...
      <div className={styles.paginationControls}>
        <label className={styles.pageSizeLabel} htmlFor="ps-sel">
          Rows:
        </label>
        <select
          id="ps-sel"
...
const BulkActionsBar: React.FC<{
  ...
  onClear: () => void;
}> = ({ Tabs, count, onAction, onClear }) => (
  <div className={styles.bulkBar} role="toolbar" aria-label="Bulk actions">
    <span className={styles.bulkCount}>
      <strong>{count}</strong> provider{count !== 1 ? "s" : ""} selected
    </span>
    <div className={styles.bulkActions}>
      {Tabs.map((a) => (
        <button
          key={a}
          className={`${styles.bulkBtn} ${a === "Deactivate" ? styles.bulkBtnDanger : a === "Export" ? styles.bulkBtnExport : ""}`}
          onClick={() => onAction(a)}
        >
          {a}
        </button>
...
const AllProvidersTable: React.FC<AllProvidersTableProps> = ({
  ...
  const allSelected =
    providers.length > 0 && providers.every((p) => selected.has(p.id));
      ...
      <table className={styles.table} aria-label="All providers">
        ...
        <tbody>
          {providers.map((p) => (
            <tr
              key={p.id}
              className={`${styles.tr} ${selected.has(p.id) ? styles.trSelected : ""}`}
            >
              ...
              <td className={styles.td}>
                <span className={styles.specialty}>{p.specialty}</span>
              </td>
              <td className={styles.td}>
                <span className={styles.mono}>{p.npi}</span>
              </td>
              <td className={styles.td}>
                <span className={styles.mono}>{p.license}</span>
              </td>
              <td className={styles.td}>{p.state}</td>
              <td className={styles.td}>
                <span className={styles.experience}>{p.experience} yrs</span>
              </td>
              <td className={styles.td}>
                <Badge label={p.status} type={getStatusType(p.status)} />
              </td>
              <td className={styles.td}>
                <span className={styles.lastActive}>{p.lastActive}</span>
              </td>
              <td className={styles.td}>
                <Badge
                  label={p.availability}
                  type={getAvailType(p.availability)}
                />
              </td>
              <td className={styles.td}>
                <div className={styles.actionsCell}>
                  ...
                  {p.status === "Active" && (
                    <button
                      className={styles.actionBtnSuspend}
                      onClick={(e) => {
                        e.stopPropagation();
                        onSuspend(p);
                      }}
                  ...
                  {(p.status === "Suspended" || p.status === "Inactive") && (
                    <button
                      className={styles.actionBtnActivate}
                      onClick={(e) => {
                        e.stopPropagation();
                        onActivate(p);
                      }}
                  ...
                  {p.status !== "Pending" ? (
                    <div className={styles.moreWrap}>
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setOpenMoreId(openMoreId === p.id ? null : p.id);
                        }}
                        aria-label="More actions"
                        aria-expanded={openMoreId === p.id}
                      >
                        ⋯
                      </button>
                      {openMoreId === p.id && (
...
const ApprovalsTable: React.FC<ApprovalsTableProps> = ({
  ...
  const getDocBadge = (s?: DocumentStatus) =>
    s === "Complete" ? "active" : s === "Incomplete" ? "suspended" : "pending";
  const getVerBadge = (s?: VerificationStatus) =>
    s === "Verified" ? "active" : s === "Failed" ? "suspended" : "pending";
  return (
        ...
        <tbody>
          {pending.map((p) => (
            <tr key={p.id} className={styles.tr}>
              ...
              <td className={styles.td}>
                <Badge
                  label={p.documentStatus || "–"}
                  type={getDocBadge(p.documentStatus)}
                />
              </td>
              <td className={styles.td}>
                <Badge
                  label={p.verificationStatus || "–"}
                  type={getVerBadge(p.verificationStatus)}
                />
              </td>
              <td className={styles.td}>
                <div className={styles.actionsCell}>
...
const CredentialingTable: React.FC<{ providers: Provider[] }> = ({
  providers,
}) => {
            ...
            <tr key={p.id} className={styles.tr}>
              <td className={styles.td}>
                <div className={styles.providerCell}>
                  ...
                  <div>
                    <span className={styles.providerName}>{p.name}</span>
                    <span className={styles.providerEmail}>{p.specialty}</span>
                  </div>
                </div>
              </td>
              <td className={styles.td}>
              ...
              <td className={styles.td}>
                <span
                  className={`${styles.expiry} ${p.credentialStatus === "Expired" ? styles.expiryExpired : p.credentialStatus === "Expiring Soon" ? styles.expiryWarn : ""}`}
                >
                  {p.licenseExpiry}
                </span>
              </td>
              <td className={styles.td}>
                <span className={styles.mono}>{p.deaNumber || "–"}</span>
              </td>
              <td className={styles.td}>{p.boardCertification || "–"}</td>
              <td className={styles.td}>
                <Badge
                  label={p.malpracticeInsurance || "–"}
                  type={
                    p.malpracticeInsurance === "Active" ? "active" : "suspended"
                  }
                />
              </td>
              <td className={styles.td}>
                <Badge
                  label={p.credentialStatus || "–"}
                  type={getCredBadge(p.credentialStatus)}
                />
              </td>
            </tr>
          ))}
...
const AvailabilityTable: React.FC<{ providers: Provider[] }> = ({
  providers,
}) => {
        ...
        <tbody>
          {active.map((p) => (
            <tr key={p.id} className={styles.tr}>
              ...
              <td className={styles.td}>
                <span className={styles.timezone}>{p.timezone || "–"}</span>
              </td>
            </tr>
          ))}
        </tbody>
...
const SuspendedTable: React.FC<SuspendedTableProps> = ({
  providers,
  onActivate,
  onEdit,
}) => {
  ...
  if (suspended.length === 0)
    return <EmptyState message="No suspended providers" />;
  return (
    <div className={styles.tableWrapper}>
        ...
        <tbody>
          {suspended.map((p) => (
            <tr key={p.id} className={styles.tr}>
              ...
              <td className={styles.td}>
                <span className={styles.suspendedReason}>
                  {p.suspendedReason}
                </span>
              </td>
              <td className={styles.td}>{p.suspendedBy}</td>
              <td className={styles.td}>
                <Badge label="Suspended" type="suspended" />
              </td>
              <td className={styles.td}>
                <div className={styles.actionsCell}>
                  {/* <button className={styles.actionBtn} onClick={(e) => e.stopPropagation()}>View</button> */}
...
const Providers: React.FC = () => {
  ...
  const handleEditProvider = useCallback(
    (data: ProviderFormData) => {
      ...
      updateProvider(selectedProvider.id, {
        ...
        state: data.state,
        experience:
          parseInt(data.experience, 10) || selectedProvider.experience,
        status: data.status,
        availability: data.availability,
      });
      closeModal();
      ...
      if (action === "Activate") {
        ...
        ids.forEach((id) =>
          updateProvider(id, {
            status: "Suspended",
            suspendedDate: today,
            suspendedBy: "Admin (Bulk)",
            suspendedReason: "Bulk administrative action",
          }),
        );
  ...
  const resetFilters = () => {
    ...
    setFilterAvail("All Availability");
    setCurrentPage(1);
  };
  const hasFilters = !!(
    searchQuery ||
  ...
  const selectedProviders = providers.filter((p) => selectedRows.has(p.id));
  //const allSelectedPending = selectedProviders.length > 0 && selectedProviders.every((p)=> p.status === "Pending");
  const anySelectedPending =
    selectedProviders.length > 0 &&
    selectedProviders.some((p) => p.status === "Pending");

  const actionTabs = anySelectedPending
    ? []
    : ["Suspend", "Deactivate", "Export"];
    ...
    <div className={styles.page}>
      ...
      {modal === "edit" && selectedProvider && (
        <ProviderForm
          ...
          initial={{
            ...
            experience: String(selectedProvider.experience),
      ...
      {modal === "confirm-suspend" && selectedProvider && (
        <ConfirmDialog
          ...
          onConfirm={() => handleSuspend(selectedProvider)}
          onClose={closeModal}
        />
      )}
      {modal === "confirm-activate" && selectedProvider && (
        <ConfirmDialog
          ...
          onConfirm={() => handleActivate(selectedProvider)}
          onClose={closeModal}
        />
      )}
      {modal === "confirm-deactivate" && selectedProvider && (
        <ConfirmDialog
          ...
          confirmClass={styles.btnDanger}
          onConfirm={() => handleDeactivate(selectedProvider)}
          onClose={closeModal}
        />
      )}
      {modal === "confirm-approve" && selectedProvider && (
        <ConfirmDialog
          ...
          confirmClass={styles.btnSuccess}
          onConfirm={() => handleApprove(selectedProvider)}
          onClose={closeModal}
        />
      )}
      {modal === "confirm-reset-password" && selectedProvider && (
        <ConfirmDialog
          ...
          onConfirm={() => showToast("Password reset email sent")}
          onClose={closeModal}
        />
      )}
      ...
      <div className={styles.pageHeader}>
        <div className={styles.pageHeaderLeft}>
          ...
          <p className={styles.pageSubtitle}>
            Manage healthcare providers, credentials, and availability
          </p>
        </div>
        <div className={styles.pageHeaderRight}>
          <button
            ...
            aria-label="Export all providers"
          >
            <svg
              width="16"
              height="16"
      ...
      <div className={styles.controls}>
        <div className={styles.searchWrap}>
          ...
          <input
            ...
            type="search"
            placeholder="Search by name, email, NPI or license…"
            value={searchQuery}
            onChange={(e) => {
      ...
      {/* ── Bulk Actions ── */}
      {selectedRows.size > 0 && !anySelectedPending && (
        <BulkActionsBar
          Tabs={actionTabs}
        ...
        >
          {TABS.map((tab, i) => (
            ...
            >
              {tab}
              <span
                className={`${styles.tabCount} ${activeTab === i ? styles.tabCountActive : ""}`}
              >
            ...
            {activeTab === 0 &&
              (paginated.length === 0 ? (
                ...
                <AllProvidersTable
                  ...
                  onResetPassword={(p) =>
                    openModal("confirm-reset-password", p)
                  }
